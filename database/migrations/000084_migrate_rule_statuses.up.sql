...
WHERE re.rule_entity_id IS NULL;
...
-- Set the rule_entity_id column in the rows we migrated from
-- rule_evaluations.
...
SET rule_entity_id = id
WHERE id IN (SELECT rule_evaluation_id FROM temp_migrate_rule_evaluations);

-- Make rule_entity_id mandatory in rule_evaluations.
-- At this point - all statuses have been migrated over.
ALTER TABLE rule_evaluations ALTER COLUMN rule_entity_id SET NOT NULL;
