...
ON CONFLICT(rule_eval_id)
    DO UPDATE SET
                  status = CASE WHEN $2 != 'skipped' AND rule_details_alert.status != 'skipped' THEN $2 ELSE rule_details_alert.status END,
                  details = CASE WHEN $2 != 'skipped' AND rule_details_alert.status != 'skipped' THEN $3 ELSE rule_details_alert.details END,
                  metadata = CASE WHEN $2 != 'skipped' AND rule_details_alert.status != 'skipped' THEN sqlc.arg(metadata)::jsonb ELSE rule_details_alert.metadata END,
                  last_updated = NOW()
