...
| email | [string](#string) |  | email is the email address of the invited user. This is presented as a convenience for display purposes, and does not affect who can accept the invitation using the nonce. |
| project | [string](#string) |  | project is the project to which the user is invited. |
| nonce | [string](#string) |  | nonce is a unique identifier for the invitation, which can be used by the recipient to accept or reject the invitation. The nonce is only transmitted in response to AssignRole or ListInvitations RPCs, and not transmitted in ListRoleAssignments or other calls. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| nonce | [string](#string) |  | nonce is the nonce of the invitation to resolve. |
