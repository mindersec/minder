...
// Package events provide a high-level interface for interacting with the eventing system
package events
...
type eventer struct {
	...
	msgInstruments *events.MessageInstruments
...
// New creates an eventer object which isolates the watermill setup code
func New(ctx context.Context, cfg *serverconfig.EventConfig) (Interface, error) {
	if cfg == nil {
		return nil, fmt.Errorf("event config is nil")
	}
	...
	metricsBuilder := metrics.NewPrometheusMetricsBuilder(
		...
		events.MetricsNamespace,
		events.MetricsSubsystem)
	metricsBuilder.AddPrometheusRouterMetrics(router)
	zerolog.Ctx(ctx).Info().Msg("Router Metrics registered")

	meter := otel.Meter("eventer")
	metricInstruments, err := events.InitMetricsInstruments(meter)
	if err != nil {
		return nil, err
	}
	zerolog.Ctx(ctx).Info().Msg("Metrics Instruments registered")

	pub, sub, cl, err := events.InstantiateDriver(ctx, cfg.Driver, cfg)
	if err != nil {
		return nil, fmt.Errorf("failed instantiating driver: %w", err)
	}

	poisonQueueMiddleware, err := middleware.PoisonQueue(pub, events.DeadLetterQueueTopic)
	if err != nil {
		return nil, fmt.Errorf("failed instantiating poison queue: %w", err)
	}
	// Router level middleware are executed for every message sent to the router
	router.AddMiddleware(
		events.RecordMetrics(metricInstruments),
...
// Publish implements the message.Publisher
func (e *eventer) Publish(topic string, messages ...*message.Message) error {
	...
	if ok && details != nil {
		for idx := range messages {
			...
			msg.Metadata.Set(events.PublishedKey, time.Now().Format(time.RFC3339))
		}
	}
