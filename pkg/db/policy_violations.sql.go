...
const getPolicyViolations = `-- name: GetPolicyViolations :many
...
type GetPolicyViolationsRow struct {
...
func (q *Queries) GetPolicyViolations(ctx context.Context, arg GetPolicyViolationsParams) ([]GetPolicyViolationsRow, error) {
	rows, err := q.db.QueryContext(ctx, getPolicyViolations,
		arg.Provider,
		arg.GroupID,
		arg.Limit,
		arg.Offset,
	...
	defer rows.Close()
	items := []GetPolicyViolationsRow{}
	for rows.Next() {
		var i GetPolicyViolationsRow
		if err := rows.Scan(
			&i.PolicyType,
			&i.RepoID,
			&i.RepoOwner,
