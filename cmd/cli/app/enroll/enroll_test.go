...
func TestCobraMain(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "enroll command",
			args:           []string{"enroll", "--help"},
			expectedOutput: "enroll called\n",
		},
		{
			name:           "enroll integration command",
			args:           []string{"enroll", "integration", "--help"},
			expectedOutput: "enroll integration called\n",
		},
	}
