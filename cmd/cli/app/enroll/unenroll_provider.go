...
var unenroll_providerCmd = &cobra.Command{
	Use:   "provider",
...
func init() {
	UnenrollCmd.AddCommand(unenroll_providerCmd)
	unenroll_providerCmd.Flags().StringP("provider", "n", "", "Name for the provider to unenroll")
	unenroll_providerCmd.Flags().BoolP("all", "a", false, "Revoke all tokens")
	unenroll_providerCmd.Flags().Int32P("group-id", "g", 0, "Group ID to revoke tokens for")

	if err := unenroll_providerCmd.MarkFlagRequired("provider"); err != nil {
