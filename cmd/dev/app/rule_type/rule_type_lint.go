...
// lintCmd is the root command for the rule subcommands
...
func lintCmdRun(cmd *cobra.Command, _ []string) error {
	...
	if rt.Def == nil {
		return fmt.Errorf("rule type definition is nil")
	}

	if err := engine.ValidateRuleTypeDefinition(rt.Def); err != nil {
		return fmt.Errorf("failed validating rule type definition: %w", err)
	}
...
func validateRegoRule(ctx context.Context, r *mediatorv1.RuleType_Definition_Eval_Rego, path string, out io.Writer) error {
	if r == nil {
