...
func (rp *ReminderProcessor) Register(r events.Registrar) {
	r.Register(reminderevents.RepoReminderEventTopic, rp.reminderMessageHandler,
		events.ReminderSubscriber)
}

func (rp *ReminderProcessor) reminderMessageHandler(msg *message.Message) error {
	evt, err := reminderevents.RepoReminderEventFromMessage(msg)
	if err != nil {
	...
	repoReconcileMsg, err := reconcilers.NewRepoReconcilerMessage(evt.Provider, evt.Repository, evt.Project)
	if err != nil {
		return fmt.Errorf("error creating repo reconcile event: %w", err)
	}

	// This is a non-fatal error, so we'll just log it and continue with the next ones
	if err := rp.evt.Publish(reconcilers.InternalReconcilerEventTopic, repoReconcileMsg); err != nil {
