...
func TestEvaluationDetailRendering(t *testing.T) {
	...
	tests := []struct {
		...
		// rego/deny-by-default template
		{
			...
			args:    map[string]any{"status": "foo", "message": "bar", "entityName": ""},
			error:   "evaluation failure: this is the message",
			details: "Evaluation foo: bar",
		},
		{
			name:    "optional entity name",
			msg:     "this is the message",
			tmpl:    templates.RegoDenyByDefaultTemplate,
			args:    map[string]any{"status": "foo", "message": "bar", "entityName": "baz"},
			error:   "evaluation failure: this is the message",
			details: "Evaluation foo: bar for entity baz",
		},
		{
