...
func (e *Evaluator) Eval(
	...
	if err != nil {
		return &interfaces.EvaluationResult{
			Error: fmt.Errorf("reading pull request dependencies: %w", err),
		}
	}
	...
	if err != nil {
		return &interfaces.EvaluationResult{
			Error: fmt.Errorf("parsing policy configuration: %w", err),
		}
	}
	...
	if err != nil {
		return &interfaces.EvaluationResult{
			Error: fmt.Errorf("failed to create summary handler: %w", err),
		}
	}
	...
	for _, dep := range prDependencies.Deps {
		...
		if err != nil {
			...
			return &interfaces.EvaluationResult{
				Error: fmt.Errorf("getting dependency score: %w", err),
			}
		}
	...
	if err := submitSummary(ctx, prSummaryHandler, ruleConfig); err != nil {
		...
		return &interfaces.EvaluationResult{
			Error: fmt.Errorf("submitting pull request summary: %w", err),
		}
	}

	return &interfaces.EvaluationResult{
		Error: buildEvalResult(prSummaryHandler),
	}
}
