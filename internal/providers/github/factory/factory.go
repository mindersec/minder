...
// GitHubProviderFactory describes a struct which can create instances of the
// GitHub provider
type GitHubProviderFactory interface {
	// Build creates an instance of `v1.Github` from a `db.Provider`
	Build(ctx context.Context, config *db.Provider) (v1.GitHub, error)
}

// NewGitHubProviderFactory creates an instance of GitHubProviderFactory
...
func (g *githubProviderFactory) Build(ctx context.Context, config *db.Provider) (v1.GitHub, error) {
	if config.CanImplement(db.ProviderTypeGithub) {
	...
	if config.Name == ghclient.Github {
