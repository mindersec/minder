...
func (r *repositoryService) DeleteByID(ctx context.Context, repositoryID uuid.UUID, projectID uuid.UUID) error {
	...
	ent, err := r.propSvc.EntityWithProperties(ctx, repositoryID, projectID, nil)
	if err != nil {
...
func (r *repositoryService) DeleteByName(
	...
	// This fetches all properties for the repository
	ent, err := r.propSvc.EntityWithProperties(ctx, repo.ID, projectID, nil)
	if err != nil {
		return fmt.Errorf("error fetching repository: %w", err)
	}
